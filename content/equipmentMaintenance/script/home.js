...
function funPtSearchPara() {
	...
	jsonData = setJson(jsonData, "repairUser",$("#checkUser").val());
	if(!$("#startTime").val()){
	...
	jsonData = setJson(jsonData, "queryCycle", parseInt(clickEle));
	// jsonData = setJson(jsonData, "isAccept", parseInt($("#yanshou").val()));
	jsonData = setJson(jsonData, "isAccept", -1);
...
function funPtSearch() {
	...
	$.ajax({
		type: "post",
		dataType: 'json',
		url: "/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsRepairBasicInfoSearchCmd",
		contentType: "application/text,charset=utf-8",
