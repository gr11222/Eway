...
function funGetRepairInfoPara(){
	...
	var jsonData = setJson(null,"requestCommand","");
		...
		jsonData = setJson(jsonData,"nameCn",$("#userName").val());
		jsonData = setJson(jsonData,"nameEn",$("#userAccount").val());
...
//		jsonData = setJson(jsonData,"number",data_number);
		console.log("查询工作量统计信息传值="+jsonData);
		return jsonData;
}
function funGetRepairInfo(){
	...
	$.ajax({
		type:"post",
		dataType:'json',
		url:"/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsCheckCountSearchCmd",
		contentType:"application/text,charset=utf-8",
		data:funGetRepairInfoPara(),
		success:function(msg){		
			loadingStop();
//			windowRemove();
			console.log("查询工作量信息返回值="+JSON.stringify(msg));	
			$("#ptcontent").html("");
			$("#pageNumId").val("");
		...
		error:function(XMLHttpRequest, textStatus, errorThrown){
            ...
            if(xmlRequest=="401"){
                 windowStart("当前用户无查询工作量统计信息权限",false);
            }else{
            	windowStart("查询工作量统计信息失败",false);
            }
...
//	
	...
	str += "<th class='text-center'><div class='checkbox'>用户账号</div></th>";
	str += "<th class='text-center'><div class='checkbox'>巡检设备总数</div></th>";
	str += "<th class='text-center'><div class='checkbox'>最后一次巡检时间</div></th>";
	str += "</thead><tbody>";
	for( var i = 0 ; i < realData.length ; i++ )
	{
		...
		str += "<td class='text-center'  style='width:20%;word-wrap: break-word;word-break: break-all;'>"+realData[i]["nameEn"]+"</td>";
		str += "<td class='text-center'  style='width:15%;word-wrap: break-word;word-break: break-all;'>"+realData[i]["count"]+"</td>";
		str += "<td class='text-center'  style='width:35%;word-wrap: break-word;word-break: break-all;'>"+realData[i]["dateTime"]+"</td>";
		str += "</tr>";
	}
	str += "</tbody><table>";
	$("#ptcontent").html(str);
...
function funExportInfo(){
	...
	$.ajax({
		type:"post",
		dataType:'json',
		url:"/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsCheckCountExportCmd",
		contentType:"application/text,charset=utf-8",
		data:funGetRepairInfoPara(),
		success:function(msg){		
			
			console.log("导出工作量统计信息返回值="+JSON.stringify(msg));	
			if(msg.responseCommand.toUpperCase().indexOf("FAIl") != -1)
			{
				windowStart("导出工作量统计信息失败",false);
			}
			else if( msg.responseCommand.toUpperCase().indexOf("NODATA") != -1)
			{
				windowStart("当前条件下无工作量统计信息",false);
			}
			else
			{
		...
		error:function(XMLHttpRequest, textStatus, errorThrown){
			...
			var xmlRequest = XMLHttpRequest.status;
            if(xmlRequest=="401"){
                 windowStart("当前用户无导出工作量统计信息权限",false);
            }else{
            	windowStart("导出工作量统计信息失败",false);
            }
//			windowStart("查询点检信息失败",false);
...
function printDeviceInfo(){
	...
	var htmlTitle = "<html><head><meta charset='utf-8'><title>巡检工作量统计打印</title></head><body>";
	var htmlhead = "";
	var str = "";
    htmlhead += "<div>";
