...
function printWeiBaoReport(){
	...
	var htmlTitle = "<html><head><title>投诉记录统计</title></head><body>";
...
//				windowRemove();
//			}
			console.log("查询投诉返回值="+JSON.stringify(msg));	
			$("#weibaoDataContent").html("");
			$("#pageNumId2").val("");
		...
		error:function(XMLHttpRequest, textStatus, errorThrown){
			loadingStop();
//			alert("查询投诉信息失败");
//			windowStart("查询投诉信息失败",false);
			var xmlRequest = XMLHttpRequest.status;
            if(xmlRequest=="401"){
                 windowStart("当前用户无查询投诉信息权限",false);
            }else{
            	windowStart("查询投诉信息失败",false);
            }
		}
...
//	str += "<th class='text-center td-width-2 '>标签型号</th>";
	str += "<th class='text-center td-width-2' style='width:12%'>投诉记录</th>";
	str += "<th class='text-center td-width-2'>投诉时间</th>";
	str += "<th class='text-center td-width-2'>投诉人</th>";
	str += "</thead><tbody>";
...
function funExportWbInfo(){
	...
	$.ajax({
		...
		success:function(msg){		
			loadingStop();
			console.log("导出投诉返回值="+JSON.stringify(msg));	
		...
		error:function(XMLHttpRequest, textStatus, errorThrown){
			loadingStop();
//			alert("导出点检统计信息失败");
//			windowStart("导出投诉记录统计信息失败",false);
			var xmlRequest = XMLHttpRequest.status;
            if(xmlRequest=="401"){
                 windowStart("当前用户无导出投诉记录统计信息权限",false);
            }else{
            	windowStart("导出投诉记录统计信息失败",false);
            }
		}
