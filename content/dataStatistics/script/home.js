...
function searchUserInfosPara() {
	//	var query=setJson(null,"propertyId",parseInt($("#company").val()));
	//	query = setJson(query, "projectId", parseInt($("#project").val()));
	//	var jsonData = setJson(null, "query", eval("("+query+")"));
...
function searchUserInfos() {
	...
	$.ajax({
		type: "post",
		dataType: 'json',
		url: "/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsDataStatisticsQueryCriteriaCmd",
		contentType: "application/text,charset=utf-8",
...
function dataStatisticsSearchPara() {
	...
	if(!$("#userName").val()) {
		jsonData = setJson(jsonData, "account", "");
	} else {
...
function createDataStatisticsTableInfos(userInfos) {
	...
	for(var i = data_number * (curren_page - 1); i < topNum; i++) {
		...
		for(var m = 0; m < 6; m++) {
			switch(data[m]["sign"]) {
				case 1:
					tableContent[m] = "<td class='text-center' style='width:10%'><a style='cursor:pointer' class='checkInfos' sign='" + data[m]["sign"] + "' user='" + realData[i].account + "' username='" + realData[i].nameCn + "'>" + data[m]["count"] + "</a></td>";
					break;
				case 2:
					tableContent[m] = "<td class='text-center' style='width:10%'><a style='cursor:pointer' class='checkInfos' sign='" + data[m]["sign"] + "' user='" + realData[i].account + "' username='" + realData[i].nameCn + "'>" + data[m]["count"] + "</a></td>";
					break;
		...
		str += "<td class='text-center' style='width:10%'>" + realData[i].nameCn + "</td>";
		for(var n = 0; n < 6; n++) {
			str += tableContent[n];
		}
		str += "</td>"
	...
	$(".checkInfos").click(function() {
		...
		switch(parseInt(sign)) {
			case 1:
				$("#lable").text("资产名称");
				$("#time").text("巡检时间");
				$("#headLine").text("巡检时间统计");
				break;
			case 2:
				$("#lable").text("资产名称");
				$("#time").text("维保时间");
				$("#headLine").text("维保时间统计");
				break;
			case 3:
				$("#lable").text("资产名称");
				$("#time").text("提交时间");
