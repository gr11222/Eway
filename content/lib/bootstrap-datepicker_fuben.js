...
!function( $ ) {
	...
	var Datepicker = function(element, options){
		...
		this.format = DPGlobal.parseFormat(options.format||this.element.data('date-format')||'yyyy-mm-dd'||'yyyy-mm'||'yyyy');
	...
	Datepicker.prototype = {
		...
		setYstValue: function() {
			var formated = DPGlobal.formatYesDate(this.date, this.format);
			if (!this.isInput) {
				if (this.component){
					this.element.find('input').prop('value', formated);
				}
	...
	var DPGlobal = {
		...
		parseFormat: function(format){
			...
			if(format.indexOf("-")!= -1){
				...
				return {separator: separator, parts: ["yyyy"]};
			}
		...
		formatYesDate: function(date, format){
			var val = {
				d: date.getDate()-1,
				m: date.getMonth() + 1,
				yy: date.getFullYear().toString().substring(2),
