...
!function( $ ) {
	...
	Datepicker.prototype = {
		...
		setYesterdayValue: function() {
			var the_time = 24*60*60*1000;
			var times = parseInt(this.date.getTime()) - the_time;
			var theDate = new Date(times)
		
			var formated = DPGlobal.formatYesterdayDate(theDate, this.format);
			if (!this.isInput) {
			...
			if (target.length == 1) {
				switch(target[0].nodeName.toLowerCase()) {
					...
					//日期
						if (target.is('.day')){
//							var the_time = 24*60*60*1000;
//							var time = parseInt(this.date.getTime())- parseInt(the_time);
...
//							
	...
	var DPGlobal = {
		...
		formatYesterdayDate: function(date, format){
