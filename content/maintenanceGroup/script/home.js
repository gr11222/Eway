...
function maintenanceGroupSearchPara() {
...
function funDeviceList(id) {
	$.ajax({
		type: "post",
		dataType: 'json',
		url: "/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsDataStatisticsGroupByRepairCmd",
		contentType: "application/text,charset=utf-8",
...
function createDeviceInfos(msg) {
	...
	//str += "<th class='' ><div class='CheckBox'><label for='allCheckBox' style='font-weight:bold'><input type='checkbox' id='allCheckBox'>全选</label></div></th>";
		...
		str += "<th class='text-center'>维保时间</th>";
	//	str += "<th class='text-center'>"+headData[i]["nameCn"]+"</th>";
	...
	for(var i = 0; i < realData.length; i++) {
		...
		str += "<td class='text-center' style='width: 25%;word-wrap: break-word;word-break: break-all;'>" + realData[i]["labelId"] + "</td>";
		str += "<td class='text-center' style='width: 25%;word-wrap: break-word;word-break: break-all;'>" + realData[i]["note"] + "</td>";
       str += "<td class='text-center' style='width: 25%;word-wrap: break-word;word-break: break-all;'>" + realData[i]["checkTime"] + "</td>";
		str += "</tr>";
	}
	str += "</tbody><table>";
	$("#listPart").html(str);
