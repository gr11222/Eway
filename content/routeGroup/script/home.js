...
function funDeviceList(id) {
	$.ajax({
		type: "post",
		dataType: 'json',
		url: "/DevOpsNoSpring/servlet/DevOpsService?cmd=DevOpsDataStatisticsGroupByCheckCmd",
		contentType: "application/text,charset=utf-8",
...
function createDeviceInfos(msg) {
	...
	str += "</tr></thead><tbody>";
	for(var i = 1; i < realData.length; i++) {
		var theData = realData[i]["items"];
		str += "<tr class='the-qrcode-class' >";
//		str += "<td class='text-center' style='width: 33%;word-wrap: break-word;word-break: break-all;'>" + realData[i]["nameCn"] + "</td>";
		for(var m=0;m<theData.length;m++){
		str += "<td class='text-center' style='width: 10%;word-wrap: break-word;word-break: break-all;'>" + theData[m]["defaultValue"] + "</td>";	
		}
